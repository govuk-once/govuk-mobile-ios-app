...
class NotificationCentreViewModel: ObservableObject {
    ...
    struct MockData {
        static let testNotifications: [Notification] = {
            ...
            return [.init(id: "1", title: "Test 1 with a really really really long title that will surely be chopped off if we add enough filler text to the end so it goes to more than two lines", body: "Body 1", date: now, status: "UNREAD", messageTitle: nil, messageBody: nil),
